...
test_content = <<~'RUBY'
  RSpec.describe "Enriched JSON Output Examples" do
    ...
    it "eq with nil objects" do
      expect(nil).to eq("foo")
    end
...
Tempfile.create(["demo_test", ".rb"]) do |test_file|
  ...
  Tempfile.create(["enriched_output", ".json"]) do |output_file|
    ...
    categories = {
      "Basic Equality Matchers" => ["eq with strings", "eq with numbers", "eq with arrays", "eq with hashes", "eq with nested structures", "eq with nil objects"],
